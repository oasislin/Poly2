...
│  (12 站 × 4 季 × 5 节点 = 240 组独立矩阵模型 + 时效插值)     │
└─────────────────────────────────────────────────────────────┘
                               ▲
┌─────────────────────────────────────────────────────────────┐
...
│  • 12 站统一特征库 (data/processed/gefs_factors/, 240 文件) │
...
根据《ADR-0007》及《项目方案 (v2.6)》，系统已从 Phase 1 的局部旧站全面升级为美国 12 站点宇宙体系：
...
6. **扩充/储备站**：`KAUS`（奥斯汀）、`KDCA`（华盛顿里根，前瞻观察站）
...
| **Phase 1.5: Task 09** | **GEFS 补全与 12 站特征提取**<br>• 外部冷存储 292,200 个 GRIB2 全球场补全与内容去重<br>• 12 站 × 20 年 (2000–2019) 因子长表提取（共 10,518,360 行）<br>• V1~V13 全部 13 项门禁 100% 验收通过 | **100%** | `gefs_factors/` (240 个 Parquet)<br>`gefs_task09_qc_v1.0.md`<br>`manifest.json` |
