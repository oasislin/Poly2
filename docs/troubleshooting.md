...
### 1.3 本地代理断开与 `CLOSE_WAIT` 连接堆积 (已对齐 v5.9.2)
- **现象**：高并发拉取多个成员切片时，出现大量 `CLOSE_WAIT` 套接字挂起，或下载数天后连接卡死。
- **根因分析**：
  1. 本地代理工具对空闲 HTTP Keep-Alive 连接单方面断开，但 Client 端套接字未及时被 EOF 释放；
  2. 直连 AWS S3（`use_proxy: false`）性能优于本地代理（实测直连 793ms vs 代理 862ms），且彻底免除了中间代理断连。
- **处置 SOP**：
  1. 在 `configs/default.yaml` 中配置 `network.use_proxy: false`（默认纯直连）；
  2. 启动脚本附带 `--no-proxy` 或直接使用默认配置。

### 1.4 中途中断恢复与 `.nc` / `.md5` 双重原子保障 (已对齐 v5.9.2)
- **现象**：批量下载过程中使用 `Ctrl + C` 或系统重启中断。
- **保护机制**：
  1. 单日数据下载具有**原子落盘特性**：只有当 5 成员全部 10 个切片拉齐并裁剪合并为 `xarray.Dataset` 后，才会写入 `.nc` 并同时生成 `.md5` 校验文件；
  2. 重启运行时，批量下载器校验 `.nc` 与 `.md5` 均完整匹配才跳过，未完成的单日自动完全重新拉取，**绝不留存半截破损数据**。

### 1.5 5 槽位实时流速与 30s 周期增量测速机制 (已对齐 v5.9.2)
- **现象**：心跳日志中网速显示与实际物理流速不符或卡死。
- **测速原理**：
  - 测速采用 **30 秒周期净增量模型**：
    $$\text{实时网速 (KB/s)} = \frac{\text{本周期末已下字节} - \text{上周期末已下字节 (KB)}}{\text{本周期实际经过时间 } \Delta t \text{ (约 30s)}}$$
  - 当单切片下载完毕（EOF）后，在后续心跳中增量为 0，**速度自动严谨归零（0 KB/s）**；
  - 槽位动态标注当前切片变量名（如 `c00[tmax]` 完成后自动切换为 `c00[tmin]`）。
