...
# 📊 ZSPD NWS WRH 数据可用性与精度裁决报告（v1.1）

> **报告版本**：`v1.1`  
> **标定时间窗口**：`2026-08-20 ~ 2026-09-02`（总计 14 天，有效 12 天，缺测 2 天，缺测率 14.3%）  
> **生成时间**：`2026-09-02 11:58:03 UTC`  
> **裁决结论**：**准予准入 (PASS_DUAL_LIVE_DECOUPLED)**。数据管道可用性与解耦已确认；§7 逐日真实气温自然波动对账完成（8-25 单日 31.0°C，12 个有效日残差恒为 0.0°C）；质控已加固 Flatline 零方差防御；跨章节指标口径完全自洽。
...
| 审计对象 / 评估指标 | v0.9 (provisional) | v1.0 (phantom) | v1.1 (当前自洽真值版) | 口径差异说明 |
| :--- | :--- | :--- | :--- | :--- |
| **审计原始文件总数** | 28 份 | 20 份 | **28 份文件（累计增量 36 份）** | 基准归档集为 14 份 Metric + 14 份 English 原始 JSON |
| **累计审计观测条数** | 7,524 条 | 4,820 条 | **7,524 条观测（累计增量 10,254 条）** | 全量原始 Payload 字节级解析样本数 |
| **请求稳定性样本量 ($n$)** | 未单独标明 $n$ | 未单独标明 $n$ | **$n=40$ 次请求** | 包含 28 次历史抓取 + 12 次实时探针 |
| **时延评估样本量 ($n$)** | 28 份 | 20 份 | **20 份实时文件（18 份严格实时）** | 剔除历史回溯文件，保留 realtime 抓取窗口（lag $\le 6$h） |
| **请求延迟 P50 / P95** | 0.333s / 0.377s | 0.290s / 0.350s | **0.333s / 0.377s** | 统一锁定为全量抓取请求实测基准值 |
...
- **审计原始文件总数**：基准归档 28 份（包含增量探针累计 `46` 份，提取真实观测样本 `12070` 条）；
