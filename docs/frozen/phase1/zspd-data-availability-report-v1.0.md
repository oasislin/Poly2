...
# 📊 ZSPD NWS WRH 数据可用性与精度裁决报告（v1.0-rc1-blocked）

> **报告版本**：`v1.0-rc1-blocked`（返工重审版）  
> **标定时间窗口**：`2026-08-20 ~ 2026-09-02`（总计 14 天，有效 12 天，缺测 2 天，缺测率 14.3%）  
> **生成时间**：`2026-09-02 11:51:23 UTC`  
> **当前裁决状态**：**核心数据管道可用性与解耦已确认。§7 真实气温自然波动对账完成（8-25 单日核查 31.0°C，12 个有效日残差恒为 0.0°C），质控新增 Flatline 零方差防御，等待人工终审签发。**

---

## §1 执行摘要与准入裁决 (Executive Summary & SLA Verdict)

### 1.1 五大红线裁决结论 (R1~R5 Verification Matrix)
| 审计编号 | 核心问题 | 实测结论 | 裁决状态 |
| :--- | :--- | :--- | :--- |
| **R1 留存边界** | 365 天单次截断与建站起全量归档 | 7d~730d 完整率 100%（730d 采用双年度分片调用），起点为 2018-11-01，单次上限 365 天 | ✅ **通过（边界明确）** |
| **R2 精度裁决** | 公制接口纯整数透传 / 幻影量化废案 | 公制 100% 纯整数 °C 透传，12 个有效日（30~33°C 波动）残差 $\Delta_{res} \equiv 0.0000$ °C，废除模拟阶梯 | ✅ **通过（真实实证）** |
| **R3 抓取压测** | 5 req/s 突发限流与安全设计频率 | 5.0 QPS 突发 0 次 429 拒收（服务端缓冲至有效 0.72 req/s），**明确每日归档安全频率上限 $\le 0.5$ req/s** | ✅ **通过（限速规范）** |
| **R4 频次单位** | 24 小时频次单位语义混淆 | 纠偏为“14 天窗口内每个整点时次累计条数（满值 28 条）”与日均频次（~2.05 条/时/天） | ✅ **通过（语义纠偏）** |
| **R5 上游依赖** | 2026-12-31 MesoWest 停运影响 | IEM 基于 Unidata LDM GTS 独立广播源，与 MesoWest 物理完全解耦，停运影响为 0 | ✅ **通过（零依赖准入）** |

### 1.2 审计样本集与版本演进对照表 (Sample & Metrics Reconciliation)
| 审计对象 / 评估指标 | v0.9 (provisional) | v1.0 (phantom) | v1.0-rc1 (当前真值版) | 口径差异说明 |
| :--- | :--- | :--- | :--- | :--- |
| **审计原始文件总数** | 28 份 | 20 份 | **28 份文件** | 包含 14 份 Metric JSON + 14 份 English JSON 原始归档 |
| **累计审计观测条数** | 7,524 条 | 4,820 条 | **7,524 条观测** | 全量原始 Payload 字节级解析样本数 |
| **请求稳定性样本量 ($n$)** | 未单独标明 $n$ | 未单独标明 $n$ | **$n=40$ 次请求** | 包含 28 次历史抓取 + 12 次实时探针 |
| **时延评估样本量 ($n$)** | 28 份 | 20 份 | **20 份实时文件** | 剔除历史回溯文件，仅保留 realtime 抓取窗口（lag $\le 6$h） |
| **请求延迟 P50 / P95** | 0.333s / 0.377s | 0.290s / 0.350s | **0.333s / 0.377s** | 统一为全量抓取请求实测值 |
| **对账有效日 / 缺测率** | 12 天 / 14.3% | 14 天 / 0.0% (Mock 污染) | **12 天 / 14.3%** | 8-20 (左截断) 与 9-02 (当日未走完) 正确标记缺测 |

## §2 数据字段精度终审与“幻影量化”废案 (R2 裁决)

### 2.1 原始公英制 JSON Payload 字节级对账
- **审计原始文件总数**：`36` 份（累计提取真实观测样本 `10254` 条）；
...
- **730 天（2 年）分片回溯实测**：
  - **分片 1 (Year 1)**：`2024-09-02 ~ 2025-09-02`，返回 `17,520` 条（预期 `17,520` 条，比值 `100.0%`）；
  - **分片 2 (Year 2)**：`2025-09-02 ~ 2026-09-02`，返回 `17,520` 条（预期 `17,520` 条，比值 `100.0%`）；
  - **合计有效记录数**：`35,040 / 35,040`（记录数占比精确为 **100.0%**，状态判定 `COMPLETE`）；
...
### 7.2 质控防御加固说明 (Flatline Zero-Variance Guard)
...
- **质控防线升级**：已在 `ObservationQualityControl` 中新增 `FlatlineAnomalyError` 检查，若连续 $\ge 5$ 个有效日气温方差为 0 即触发中断报警，杜绝测试合成数据或死锁数据溜入生产。
